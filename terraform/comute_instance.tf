...
resource "openstack_compute_instance_v2" "registry" {
  ...
  key_pair = var.key_pair
  security_groups = [openstack_networking_secgroup_v2.sec_ssh_grp.name, openstack_networking_secgroup_v2.icmp_allow.name, openstack_networking_secgroup_v2.registry.name]
}
...
# floating ip c for registry
resource "openstack_compute_floatingip_associate_v2" "registry_ip" {
  floating_ip = openstack_networking_floatingip_v2.registry.address
